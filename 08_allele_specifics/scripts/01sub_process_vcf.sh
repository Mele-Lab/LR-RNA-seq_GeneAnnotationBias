...
#$$ coded added by Pau Clavell-Revelles
module load bcftools gatk samtools parallel java-openjdk/22.0.1 htslib
#$$

set -eo pipefail

declare -a DEPENDENCIES=(parallel bcftools gatk)
...
function SplitVCF() {
    ...
    (set -x; bcftools view --threads 112 --force-samples -s "${donor}" "${vcf}" | bgzip -c > "${outname}")
...
function keepHet() {
    ...
    (set -x; bcftools view --threads 112 -m2 -M2 -v snps -Ov -g het ${samplename} | bgzip -c > ${outname})
    (set -x; tabix -p vcf ${outname})
}
...
(set -x; bcftools norm --threads 112 -d all -Ou -N ${VCF} | \
        bcftools view --threads 112 -m2 -M2 -v snps -S ${SAMPLES} -Ov --force-samples | \
