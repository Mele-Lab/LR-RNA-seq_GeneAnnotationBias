...
#$ -j y
#$ -t 1-59:1
#$ -o logs/kmerCounts-specRegion-$JOB_ID-$TASK_ID.out.log
#$ -e logs/kmerCounts-specRegion-$JOB_ID$TASK_ID.err.log
...
###############################################
file="[PATH]/mapping_minimap/repetitionQuantification/kmerCount_specRegion.list"
line=`sed "$((SGE_TASK_ID))q;d" ${file}`
#fasta="/nfs/no_backup/rg/fdegalez/mapping_minimap/genomic_comparison/HG01928_NA18906/unique_regions_sup1000.sorted.fasta"
...
cd [PATH]/mapping_minimap/repetitionQuantification/specRegion

#### Functions
kmer_size=6

dirname=$(dirname $fasta)
name=$(echo $dirname | awk -F "/" '{print $NF}')
