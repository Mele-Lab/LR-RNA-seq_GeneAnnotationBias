...
###### FILTERING
# Keep:
data[, filter:=ifelse((structural_category=="full-splice_match" |
       structural_category=="incomplete-splice_match" & existsFSMinTranscript==F |
       structural_category=="novel_in_catalog" & sample_sharing>1 & flair_max_counts>=3 & length >=300|
       structural_category%in%c("novel_not_in_catalog", "genic", "fusion", "antisense", "intergenic") & sample_sharing>1 & flair_max_counts>=3 & sj_less_recountsupported_counts>=25 & tool_sharing>1 & length >=300) &
